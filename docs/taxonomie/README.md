...
> **Source de vérité machine-lisible :** `roles.json` (dans ce dossier) — validé en CI par `scripts/check-taxonomie.mjs`
> (invariants de la taxonomie + couverture : chaque US/EN porte un champ `Rôle:` résolvant vers un rôle,
> un `macro:<macro-rôle>` ou un `groupe:<groupe>` déclaré ici). Ce document est la version **humaine**.
