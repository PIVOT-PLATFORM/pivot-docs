# ADR-008 — Domaines composables & cockpits par persona
...
2. **Le parcours d'un persona traverse plusieurs capacités.** Un chef de projet a besoin, au même endroit, de sa roadmap, de son budget, de ses OKR **et** de ses risques. Un PMO a besoin du portefeuille **et** de la consolidation des risques. Traiter « Pilotage » comme un module monolithique fige ces capacités ensemble et empêche de les recomposer par persona.
...
### 3. Cockpit = composition par persona

Un **cockpit** est une vue composée, propre à un persona, qui agrège les widgets et vues des modules pertinents. Il n'est pas « possédé » par un module : c'est une **composition** orchestrée par le shell (E16), alimentée par les modules via le bus et les deep-links.

Composition proposée — **personas et périmètres à définir / valider** :

| Persona | Cockpit (proposé) | Modules composés |
|---------|-------------------|------------------|
...
> ⚠️ Ce tableau est une **proposition de départ**. La liste des personas, leurs cockpits et les widgets composés restent **à définir** dans une itération produit dédiée.
...
**Inconvénient :** doublon « risque léger / risque complet », parcours utilisateur ambigu, impossible de recomposer les capacités par persona. Fige des cycles de release hétérogènes ensemble.

### Option B — Risque comme simple feature de Pilotage (rejeté)

**Inconvénient :** l'ampleur du domaine risque (40+ US, packs de conformité, IA gouvernée, boucle vivante) déborde largement une feature ; sa réutilisation par d'autres personas (PMO, sponsor, contract manager) impose un module autonome.

### Option C — Cockpits « en dur » codés dans chaque module (rejeté)

**Inconvénient :** chaque module ré-implémenterait la vue de chaque persona → duplication et couplage. Le cockpit doit être une **composition** au niveau du shell, pas un artefact interne à un module.
...
### Positives
- Parcours persona cohérent : chaque cockpit agrège les bonnes capacités au bon endroit.
- Modules activables et à release indépendante (MCO par capacité).
- Risque réutilisable par tous les personas sans le dupliquer.
