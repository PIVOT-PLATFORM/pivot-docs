# EN46.1 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Feedback » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Feedback)** :

- **Tickets soumis** — nombre de tickets de feedback créés sur la période (nombre ; granularité : tenant) ; kpiKey `feedback.submitted_tickets`
- **Votes exprimés** — nombre total de votes sur les tickets (nombre ; granularité : tenant) ; kpiKey `feedback.total_votes`
- **Taux de traitement** — part des tickets passés en « Fait » (% ; granularité : tenant) ; kpiKey `feedback.resolution_rate`
- **Délai moyen de traitement** — temps moyen entre soumission et clôture d'un ticket (jours ; granularité : tenant) ; kpiKey `feedback.avg_resolution_time`
- **Taux de participation** — part des utilisateurs ayant voté ou soumis un ticket (% ; granularité : tenant) ; kpiKey `feedback.participation_rate`

**Justification** : Exposer l'activité de feedback (soumissions, votes, traitement) comme KPI liables permet de rattacher des KR d'écoute utilisateur et de réactivité produit à des mesures réelles, et d'alimenter les tableaux de bord produit.
...
---
Item Type: Enabler · Parent: E46 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
