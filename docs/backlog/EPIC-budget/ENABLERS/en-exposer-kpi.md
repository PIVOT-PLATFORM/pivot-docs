# EN26.1 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Budget & suivi financier » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `pilotage`** (`GET /api/pilotage/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Budget & suivi financier)** :

- **Taux de consommation budgétaire** — part du budget consommé par rapport au prévu (% ; granularité : projet) ; kpiKey `budget.taux_consommation`
- **Écart budgétaire (dérive)** — écart entre réel et prévu (% ; granularité : projet) ; kpiKey `budget.ecart_derive`
- **Budget engagé** — montant engagé sur la période (montant ; granularité : projet) ; kpiKey `budget.montant_engage`
- **Alertes de dérive budgétaire** — nombre d'alertes de dépassement émises (nombre ; granularité : projet) ; kpiKey `budget.alertes_derive`
- **Flux de trésorerie prévisionnel** — solde de trésorerie projeté sur la période (montant ; granularité : projet) ; kpiKey `budget.flux_tresorerie`

**Justification** : Exposer la consommation et la dérive budgétaires permet de rattacher des KR de maîtrise financière à des mesures réelles et d'alimenter les dashboards de suivi projet en temps réel.
...
---
Item Type: Enabler · Parent: E26 · Module: pilotage · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
