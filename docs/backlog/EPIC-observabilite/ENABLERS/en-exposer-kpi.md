# EN04.7 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Observabilité » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Observabilité)** :
- **Disponibilité (health checks)** — part du temps où les health checks liveness/readiness sont au vert (% ; granularité : tenant) ; kpiKey `observabilite.health_uptime`
- **Taux d'erreur** — part des requêtes en erreur (5xx) sur l'ensemble des backends (% ; granularité : tenant) ; kpiKey `observabilite.error_rate`
- **Latence p95** — temps de réponse au 95e percentile (SLO) (ms ; granularité : tenant) ; kpiKey `observabilite.latency_p95`
- **MTTR** — temps moyen de rétablissement après alerte déclenchée (minutes ; granularité : tenant) ; kpiKey `observabilite.mttr`
- **Saturation des ressources** — taux d'utilisation max Redis/DB/JVM sur la période (% ; granularité : tenant) ; kpiKey `observabilite.resource_saturation`

**Justification** : Exposer les indicateurs de santé (disponibilité, taux d'erreur, latence, MTTR) rend l'observabilité directement liable à des KR de fiabilité (SLO) et consommable par les tableaux de bord de pilotage sans dupliquer les métriques Prometheus.
...
---
Item Type: Enabler · Parent: E04 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
