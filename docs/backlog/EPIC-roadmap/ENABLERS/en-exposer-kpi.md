# EN22.4 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Roadmap & Planification » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `pilotage`** (`GET /api/pilotage/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Roadmap & Planification)** :
- **Jalons atteints** — part des jalons livrés à la date prévue (% ; granularité : tenant/projet) ; kpiKey `roadmap.jalons-atteints`
- **Dérive de planning** — écart moyen entre dates baseline et dates réelles (jours ; granularité : tenant/projet) ; kpiKey `roadmap.derive-planning`
- **Jalons à échéance** — nombre de jalons arrivant à terme sur la période (nombre ; granularité : tenant/projet) ; kpiKey `roadmap.jalons-a-echeance`
- **Avancement du plan** — avancement pondéré des tâches du plan (% ; granularité : projet) ; kpiKey `roadmap.avancement-plan`

**Justification** : Le respect des jalons et la dérive de planning sont des mesures d'exécution que les objectifs de delivery cherchent à suivre ; les exposer permet de les lier à des KR de tenue des engagements et de les afficher dans les cockpits.
...
---
Item Type: Enabler · Parent: E22 · Module: pilotage · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
