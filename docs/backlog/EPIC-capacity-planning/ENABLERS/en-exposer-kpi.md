# EN11.2 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Capacity Planning » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `agilite`** (`GET /api/agilite/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Capacity Planning)** :

- **Taux d'utilisation de capacité** — capacité engagée rapportée à la capacité nette (% ; granularité : équipe) ; kpiKey `capacity.taux_utilisation`
- **Capacité nette** — capacité nette calculée sur la période (jours-homme ; granularité : équipe) ; kpiKey `capacity.capacite_nette`
- **Vélocité moyenne** — moyenne glissante des 3 derniers sprints (points ; granularité : équipe) ; kpiKey `capacity.velocite_moyenne`
- **Taux d'absence** — part des jours d'indisponibilité sur les jours ouvrables (% ; granularité : équipe) ; kpiKey `capacity.taux_absence`
- **Dépassements de capacité** — nombre d'alertes de dépassement de capacité émises (nombre ; granularité : équipe) ; kpiKey `capacity.depassements`

**Justification** : Exposer la capacité, la vélocité et les dépassements permet de rattacher des KR de prévisibilité de livraison à des mesures réelles et d'alimenter les dashboards de planification par équipe.
...
---
Item Type: Enabler · Parent: E11 · Module: agilite · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
