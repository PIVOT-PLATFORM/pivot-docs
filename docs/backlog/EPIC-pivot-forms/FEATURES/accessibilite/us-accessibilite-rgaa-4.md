# US42.10.1 — Accessibilité RGAA 4

**En tant que** DSI publique
**Je veux** Formulaires et builder conformes RGAA 4 / WCAG 2.1 AA ; déclaration d'accessibilité.
**Afin de** répondre au besoin « Accessibilité RGAA 4 » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Les formulaires passent l'audit RGAA ; navigation clavier/lecteur d'écran OK. | ⬜ |
| A11y : builder ET formulaires conformes RGAA 4 / WCAG 2.1 AA (clavier, lecteur d'écran, contrastes) | ⬜ |

---
Item Type: US · Parent: F42.10 · Module: forms · Phase: phase-3 · Size: L · Priority: Critical
Stage: Backlog
Rôle: directeur-des-systemes-d-information
Source: FRM-A01 · MoSCoW: Must · Origine: Vide de marché + contexte public FR
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
