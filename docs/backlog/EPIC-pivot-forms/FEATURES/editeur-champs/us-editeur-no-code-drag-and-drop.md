# US42.1.1 — Éditeur no-code drag-and-drop

**En tant que** BA
**Je veux** Construction visuelle par glisser-déposer ou blocs, sans compétence technique, avec prévisualisation.
**Afin de** répondre au besoin « Éditeur no-code drag-and-drop » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Un utilisateur crée un formulaire à 5 champs sans aide en < 10 min. | ⬜ |
| Given le module Forms, when la fonctionnalité est activée, then elle est utilisable sans code, intégrée au portail (thème `--pv-*`) et i18n FR/EN | ⬜ |

---
Item Type: US · Parent: F42.1 · Module: forms · Phase: phase-3 · Size: M · Priority: Critical
Stage: Backlog
Source: FRM-001 · MoSCoW: Must · Origine: Socle 6/6
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
