# US42.9.2 — Collaboration d'équipe

**En tant que** BA
**Je veux** Édition partagée, commentaires, collaborateurs sans surcoût de siège.
**Afin de** répondre au besoin « Collaboration d'équipe » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Plusieurs personnes éditent et commentent un formulaire. | ⬜ |
| Given le module Forms, when la fonctionnalité est activée, then elle est utilisable sans code, intégrée au portail (thème `--pv-*`) et i18n FR/EN | ⬜ |

---
Item Type: US · Parent: F42.9 · Module: forms · Phase: phase-3 · Size: M · Priority: Medium
Stage: Backlog
Rôle: business-analyst
Source: FRM-902 · MoSCoW: Should · Origine: Formbricks (seats illimités), Tally
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
