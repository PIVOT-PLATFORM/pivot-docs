# US42.5.2 — API de formulaires et réponses

**En tant que** Dev
**Je veux** API de gestion des formulaires et des réponses (lecture/écriture).
**Afin de** répondre au besoin « API de formulaires et réponses » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Un formulaire et ses réponses sont pilotables par API. | ⬜ |
| Given le module Forms, when la fonctionnalité est activée, then elle est utilisable sans code, intégrée au portail (thème `--pv-*`) et i18n FR/EN | ⬜ |

---
Item Type: US · Parent: F42.5 · Module: forms · Phase: phase-3 · Size: M · Priority: High
Stage: Backlog
Rôle: developpeur
Source: FRM-402 · MoSCoW: Must · Origine: Formbricks, Jotform
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
