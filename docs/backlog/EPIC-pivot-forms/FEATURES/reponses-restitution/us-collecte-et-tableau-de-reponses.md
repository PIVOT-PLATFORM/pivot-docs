# US42.4.1 — Collecte et tableau de réponses

**En tant que** BA
**Je veux** Stockage des réponses, tableau filtrable, export CSV/Excel.
**Afin de** répondre au besoin « Collecte et tableau de réponses » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Les réponses sont consultables, filtrables et exportables. | ⬜ |
| Given le module Forms, when la fonctionnalité est activée, then elle est utilisable sans code, intégrée au portail (thème `--pv-*`) et i18n FR/EN | ⬜ |

---
Item Type: US · Parent: F42.4 · Module: forms · Phase: phase-3 · Size: M · Priority: Critical
Stage: Backlog
Source: FRM-301 · MoSCoW: Must · Origine: Socle 6/6
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
