# US42.2.2 — Calculs et scoring (quiz)

**En tant que** BA
**Je veux** Champs calculés, scoring de quiz (outcome et score) pour l'évaluation et la formation.
**Afin de** répondre au besoin « Calculs et scoring (quiz) » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Un quiz calcule un score et un résultat selon les réponses. | ⬜ |
| Given le module Forms, when la fonctionnalité est activée, then elle est utilisable sans code, intégrée au portail (thème `--pv-*`) et i18n FR/EN | ⬜ |

---
Item Type: US · Parent: F42.2 · Module: forms · Phase: phase-3 · Size: M · Priority: High
Stage: Backlog
Source: FRM-102 · MoSCoW: Should · Origine: Typeform, Jotform, Google, Formbricks
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
