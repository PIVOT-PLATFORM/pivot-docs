# US42.7.3 — Rétention et purge des réponses

**En tant que** DPO
**Je veux** Politiques de rétention paramétrables et purge automatique des réponses.
**Afin de** répondre au besoin « Rétention et purge des réponses » de la suite Forms

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Les réponses sont purgées selon la politique définie. | ⬜ |
| Security/Gouvernance : conforme à la doctrine PIVOT (SSO, RBAC, RGPD, traçabilité, souveraineté) | ⬜ |

---
Item Type: US · Parent: F42.7 · Module: forms · Phase: phase-3 · Size: M · Priority: Medium
Stage: Backlog
Source: FRM-703 · MoSCoW: Should · Origine: Vide de marché + gouvernance données
Justification: Benchmark formulaires (Typeform/Jotform/Tally/Formbricks/Qualtrics/Google) — recentré PIVOT
Dépendances: EN42.1 (moteur & schéma de formulaire)
