# EN30.14 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Collaboration » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `collaboratif`** (`GET /api/collaboratif/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Collaboration)** :

- **Boards actifs** — nombre de tableaux collaboratifs actifs sur la période (nombre ; granularité : tenant/équipe) ; kpiKey `collaboration.active_boards`
- **Taux de réunions outillées** — part des réunions s'appuyant sur un board (% ; granularité : tenant/équipe) ; kpiKey `collaboration.tooled_meetings_rate`
- **Participants actifs par atelier** — moyenne de contributeurs actifs par atelier (nombre ; granularité : équipe/projet) ; kpiKey `collaboration.active_participants`
- **Taux d'adoption du module** — part des membres ayant utilisé la collaboration visuelle (% ; granularité : tenant/équipe) ; kpiKey `collaboration.adoption_rate`
- **Équité de participation** — indice d'équilibre des contributions en atelier (indice 0–1 ; granularité : équipe/projet) ; kpiKey `collaboration.participation_equity`

**Justification** : Exposer l'adoption et l'engagement de la collaboration visuelle comme KPI liables permet de rattacher des KR d'usage et de facilitation à des mesures réelles (boards, ateliers, équité), et d'alimenter les tableaux de bord d'adoption SI.
...
---
Item Type: Enabler · Parent: E30 · Module: collaboratif · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
