...
**Je veux** modifier le style d'un connecteur existant (libellé, couleur, forme, flèche, pointillés, épaisseur), un ou plusieurs attributs à la fois
...
| Critère | 🤖 Dev |
|---------|--------|
| Given je suis OWNER ou EDITOR et qu'un connecteur d'id C existe, when j'émets `connection:update {id:C, boardId, ...patch}` avec un sous-ensemble des champs `{label, color, shape, arrow, dashed, width}`, then seuls les champs fournis (`!== undefined`) sont mis à jour et l'objet connecteur complet est diffusé à toute la room (émetteur inclus) en `connection:updated` | ⬜ |
| Given un patch partiel (ex. `{shape:"orthogonal"}` seul), when il est appliqué, then les autres attributs du connecteur (label, color, arrow, dashed, width) restent inchangés — mise à jour strictement partielle, pas de réécriture des champs absents | ⬜ |
...
| Given les formes `{droit, courbe, orthogonal}` et flèches `{aucune, début, fin, deux}`, when une valeur de `shape`/`arrow` est fournie, then elle est acceptée comme convention applicative (aucun enum en base — `shape`/`arrow` sont des `String` libres, §1.8) ; la validation du jeu de valeurs autorisé est portée côté application (client + validation d'entrée serveur), pas par une contrainte SQL | ⬜ |
...
| A11y : le panneau de style du connecteur (sélecteurs forme/flèche/couleur/épaisseur, champ label) est entièrement pilotable au clavier (Tab, contrôles natifs `<select>`/`<input>`), chaque contrôle porte un `<label>` explicite ; le connecteur rendu reflète son style dans son `aria-label` (ex. « connecteur en pointillés de A vers B ») | ⬜ |
| Tests TI (backend) : patch mono-champ (autres champs inchangés) ; patch multi-champs ; `label:null` explicite efface le label ; patch vide → no-op sans broadcast ; id inexistant → tolérant sans exception ; VIEWER → refus ; cross-tenant → refus | ⬜ |
| Tests Vitest (frontend) : émission d'un patch partiel, réconciliation sur `connection:updated`, rendu des variantes forme×flèche×pointillés×épaisseur, effacement du label via null | ⬜ |

## Hors périmètre

- Création et suppression d'un connecteur — couvertes par US08.7.1 (`connection:create`/`connection:delete`)
- Contrainte enum en base sur `shape`/`arrow` — restent des `String` libres (§1.8) ; le jeu de valeurs autorisé est une convention purement applicative, non contrainte SQL
...
- Styles de connecteur au-delà des 6 attributs `{label, color, shape, arrow, dashed, width}` (ex. animation, dégradé) — hors scope Socle
...
- i18n : clés `whiteboard.connector.style.*` (formes, flèches, pointillés, épaisseur, label) — fr.json / en.json.
