...
| Given un import réussi, when le serveur a persisté le contenu, then il **publie un événement de domaine `collaboratif.board.imported`** sur le bus d'événements (déclenchant la livraison des webhooks abonnés à `board.imported`), en plus du broadcast STOMP `board:imported` — l'événement de bus/webhook est distinct de la notification `notify()` (celle-ci n'est pas émise, cf. Hors périmètre §6 constat 14) | ⬜ |
| Given plus de **5 imports par minute** sur un tableau, when un 6ᵉ import est posté dans la fenêtre glissante, then il est rejeté **429** (`Trop de requêtes…`) — le rate-limit d'import est une garde **permanente** active dans tous les environnements (correctif §6.16 : le POC ne l'activait qu'en production) | ⬜ |
...
- **Bus d'événements / webhooks (§2.4)** : à l'import, publier `collaboratif.board.imported` sur le bus (convention `<module>.<entité>.<action>`, ADR-025) → déclenche `deliverWebhooks('board.imported', …)` pour les intégrations abonnées. C'est le pendant PIVOT du `bus.publish` + `deliverWebhooks` du POC ; distinct de `notify()` (non émis, §6 constat 14). L'undo n'émet pas d'événement de bus (parité POC)
...
- Rate-limit d'import (**5/minute**, réponse 429) : garde **permanente** côté Socle, active dans **tous les environnements** — corrige le §6 constat 16 (le POC ne l'activait qu'en `NODE_ENV=production`). Portée côté gateway/filtre applicatif (`@fastify/rate-limit` → équivalent Spring)
