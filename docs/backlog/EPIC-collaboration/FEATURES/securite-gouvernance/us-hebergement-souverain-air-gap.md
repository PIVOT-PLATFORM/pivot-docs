# US22.9.12 — Hébergement souverain / air-gap

**En tant que** administrateur
**Je veux** bénéficier d'options d'hébergement souverain, dédié ou totalement déconnecté (air-gap)
**Afin de** répondre aux exigences des secteurs régulés

## Critères d'acceptation

| Critère | 🤖 Dev |
|---------|--------|
| Given le canevas de collaboration, when options saas souverain, then le résultat est visible et persistant pour tous les participants | ⬜ |
| Dédié | ⬜ |
| Error : given une entrée invalide ou une coupure réseau, system préserve les contributions et affiche un état cohérent | ⬜ |
| Security/Gouvernance : conforme aux politiques de sécurité et de conformité (RGPD, audit, droits d'accès) | ⬜ |

---
Item Type: US · Parent: F22.9 · Module: collaboratif · Phase: phase-3 · Size: XL · Priority: Medium
Stage: Backlog
Source: BL-076 · MoSCoW: Could · Lot: Lot 4 · Origine: Différenciant Klaxoon
Justification: Dossier §6.3 ; requis en secteur régulé
Dépendances: —
