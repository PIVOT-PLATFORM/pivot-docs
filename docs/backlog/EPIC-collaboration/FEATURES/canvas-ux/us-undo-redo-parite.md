...
| Given le focus dans un `<input>`/`<textarea>`, when j'appuie `Ctrl/Cmd+Z` (undo) ou `Ctrl/Cmd+Y` / `Ctrl/Cmd+Shift+Z` (redo), then l'undo/redo canvas **s'exécute quand même** (contrairement à la plupart des autres raccourcis qui sont ignorés en champ éditable) | ⬜ |
...
| A11y : boutons Undo/Redo = `<button>` natifs avec `aria-label="Annuler (Ctrl+Z)"` / `"Rétablir (Ctrl+Y)"` et `aria-disabled` quand la pile correspondante est vide ; les raccourcis fonctionnent au clavier y compris dans un champ éditable, sans dépendre du survol souris | ⬜ |
...
- **Raccourcis** : `Ctrl/Cmd+Z` (sans Shift) = undo ; `Ctrl/Cmd+Y` ou `Ctrl/Cmd+Shift+Z` = redo. Fonctionnent **même si le focus est dans un `<input>`/`<textarea>`** (exception au comportement des autres raccourcis, cf. US08.11.6).
