# EN14.1 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « La Roue » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `agilite`** (`GET /api/agilite/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine La Roue)** :
- **Tirages réalisés** — nombre de tirages effectués sur la période (nombre ; granularité : équipe) ; kpiKey `roue.tirages-realises`
- **Participants tirés** — nombre de participants sélectionnés via la roue (nombre ; granularité : équipe) ; kpiKey `roue.participants-tires`
- **Taux de participation** — part des membres de l'équipe présents dans les roues (% ; granularité : équipe) ; kpiKey `roue.taux-participation`
- **Couverture anti-répétition** — part des membres tirés au moins une fois sur un cycle (% ; granularité : équipe) ; kpiKey `roue.couverture-anti-repetition`

**Justification** : Les mesures de participation et d'anti-répétition de la roue reflètent l'engagement d'équipe ; les exposer permet de les rattacher à des KR d'animation et de les afficher dans les tableaux de bord.
...
---
Item Type: Enabler · Parent: E14 · Module: agilite · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
