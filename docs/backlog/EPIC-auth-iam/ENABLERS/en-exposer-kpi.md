# EN01.19 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Authentification & IAM » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `auth`** (`GET /api/auth/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Authentification & IAM)** :

- **Taux d'authentification réussie** — part des tentatives de connexion réussies (% ; granularité : tenant) ; kpiKey `auth.taux_auth_reussie`
- **Taux d'adoption MFA** — part des comptes avec MFA activé (% ; granularité : tenant) ; kpiKey `auth.taux_mfa_active`
- **Échecs de connexion** — volume de tentatives de connexion échouées sur la période (nombre ; granularité : tenant) ; kpiKey `auth.echecs_connexion`
- **Sessions actives** — nombre de sessions authentifiées actives (nombre ; granularité : tenant) ; kpiKey `auth.sessions_actives`
- **Alertes de connexion suspecte** — volume d'alertes de connexion suspecte émises (nombre ; granularité : tenant) ; kpiKey `auth.alertes_connexion_suspecte`

**Justification** : Exposer la santé et la posture de sécurité des accès (succès d'authentification, adoption MFA, connexions suspectes) permet de rattacher des KR de sécurité à des mesures réelles et d'alimenter les dashboards de conformité.
...
---
Item Type: Enabler · Parent: E01 · Module: auth · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
