# EN48.2 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Assistant IA » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Assistant IA)** :

- **Taux de réponses utiles** — part des échanges notés utiles (pouce haut) par l'utilisateur (% ; granularité : tenant) ; kpiKey `assistant.taux_reponses_utiles`
- **Taux de résolution autonome** — part des échanges résolus sans escalade vers une autre ressource (% ; granularité : tenant) ; kpiKey `assistant.taux_resolution_autonome`
- **Adoption de l'assistant** — utilisateurs actifs ayant sollicité l'assistant sur la période (nombre ; granularité : tenant) ; kpiKey `assistant.adoption`
- **Latence moyenne de réponse** — temps moyen de première réponse (ms ; granularité : tenant) ; kpiKey `assistant.latence_reponse`
- **Volume d'échanges** — nombre d'échanges traités sur la période (nombre ; granularité : tenant) ; kpiKey `assistant.volume_echanges`

**Justification** : Exposer l'adoption et la qualité perçue de l'assistant permet de piloter des KR produit (adoption, utilité) et d'alimenter les dashboards de gouvernance de l'IA transverse.
...
---
Item Type: Enabler · Parent: E48 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
