# EN41.2 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Formation & Onboarding » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Formation & Onboarding)** :

- **Taux de complétion d'onboarding** — part des parcours d'onboarding terminés (% ; granularité : tenant/module) ; kpiKey `formation.onboarding_completion`
- **Taux d'adoption par module** — part des membres ayant activé un module après onboarding (% ; granularité : tenant/module) ; kpiKey `formation.module_adoption_rate`
- **Taux d'activation (funnel)** — part des utilisateurs franchissant les étapes clés d'activation (% ; granularité : tenant) ; kpiKey `formation.activation_rate`
- **Complétion e-learning** — part des parcours e-learning menés à terme (% ; granularité : tenant) ; kpiKey `formation.elearning_completion`
- **Temps jusqu'à la première valeur** — délai jusqu'à la première action clé d'un nouvel utilisateur (jours ; granularité : tenant) ; kpiKey `formation.time_to_value`

**Justification** : Exposer la complétion et l'adoption issues de l'onboarding comme KPI liables permet de rattacher des KR d'activation et d'adoption produit à des mesures réelles, et d'alimenter les tableaux de bord de mesure de l'adoption (F41.6).
...
---
Item Type: Enabler · Parent: E41 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
