# US21.5.1 — Consolidation de portefeuille

> Stub (Lot 3 · MoSCoW Must) — ACs à détailler par PO Agent lors de Gate 1 avant implémentation.

**En tant que** PMO, Sponsor
**Je veux** « Consolidation de portefeuille »
**Afin de** piloter les risques à l'échelle du portefeuille

## Contexte

Heat map multi-projets et agrégation des risques par famille et par entité.

## Critères d'acceptation (outline — Gate 1 PO Agent)

| Critère | 🤖 Dev |
|---------|--------|
| Le PMO voit une heat map consolidée de tous les projets. | ⬜ |

---
Item Type: US · Parent: F21.5 · Module: risk · Phase: phase-3 · Size: L · Priority: High
Stage: Backlog
Rôle: groupe:instances-decision
Dépendances: US21.2.1
