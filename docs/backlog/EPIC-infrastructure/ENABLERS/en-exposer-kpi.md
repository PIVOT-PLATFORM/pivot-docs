# EN07.14 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Infrastructure & Déploiement » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Infrastructure & Déploiement)** :
- **Disponibilité de la plateforme** — taux de disponibilité des services en production (% ; granularité : tenant) ; kpiKey `infrastructure.uptime`
- **MTTR** — temps moyen de rétablissement après incident de production (minutes ; granularité : tenant) ; kpiKey `infrastructure.mttr`
- **Fréquence de déploiement** — nombre de déploiements prod réussis via `deploy.yml` sur la période (déploiements/semaine ; granularité : tenant) ; kpiKey `infrastructure.deployment_frequency`
- **Taux d'échec des déploiements** — part des déploiements prod en échec ou rollback (% ; granularité : tenant) ; kpiKey `infrastructure.deployment_failure_rate`
- **Latence p95 des requêtes** — temps de réponse au 95e percentile côté backend (ms ; granularité : tenant) ; kpiKey `infrastructure.request_latency_p95`

**Justification** : Exposer la disponibilité, le MTTR et la vélocité de déploiement rend ces indicateurs de fiabilité directement liables à des KR d'exploitation (SLO, DORA) et consommables par les tableaux de bord plateforme sans collecte parallèle.
...
---
Item Type: Enabler · Parent: E07 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
