# EN17.12 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Infrastructure multi-repo » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Infrastructure multi-repo)** :
- **Fréquence de déploiement** — nombre de déploiements réussis par repo module sur la période (déploiements/semaine ; granularité : tenant) ; kpiKey `infra.deployment_frequency`
- **Modules provisionnés** — nombre de repos modules créés et opérationnels via les templates socle (nombre ; granularité : tenant) ; kpiKey `infra.provisioned_modules`
- **Délai de mise à disposition d'un module** — temps médian entre création du repo et premier déploiement (heures ; granularité : tenant) ; kpiKey `infra.module_lead_time`
- **Taux de réussite des pipelines CI/CD** — part des pipelines verts sur l'ensemble des repos modules (% ; granularité : tenant) ; kpiKey `infra.pipeline_success_rate`
- **Fraîcheur des artefacts socle** — retard médian d'adoption de la dernière version de `pivot-core-starter` / `@pivot/ui-core` (jours ; granularité : tenant) ; kpiKey `infra.starter_version_lag`

**Justification** : Exposer ces KPI de vélocité et de santé de l'industrialisation multi-repo permet de rattacher directement des KR d'industrialisation (fréquence de déploiement, délai de mise à disposition d'un module) et d'alimenter les tableaux de bord plateforme sans dupliquer la collecte.
...
---
Item Type: Enabler · Parent: E17 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
