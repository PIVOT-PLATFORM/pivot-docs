# EN23.1 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Portefeuille projets » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `pilotage`** (`GET /api/pilotage/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Portefeuille projets)** :
- **Projets par statut** — nombre de projets ventilés par statut du portefeuille (nombre ; granularité : tenant/portefeuille) ; kpiKey `portefeuille.projets-par-statut`
- **Taux d'avancement moyen** — avancement moyen pondéré des projets du portefeuille (% ; granularité : tenant/portefeuille) ; kpiKey `portefeuille.taux-avancement-moyen`
- **Projets en retard** — nombre de projets dont la date cible est dépassée (nombre ; granularité : tenant/portefeuille) ; kpiKey `portefeuille.projets-en-retard`
- **Indice RAG** — répartition des projets par indicateur de santé Red/Amber/Green (nombre ; granularité : tenant/portefeuille) ; kpiKey `portefeuille.indice-rag`

**Justification** : Les indicateurs de portefeuille (statuts, avancement, retards, RAG) sont les mesures naturelles d'objectifs stratégiques ; les exposer permet de les lier directement à des KR et de les réutiliser dans les cockpits et tableaux de bord sans ressaisie.
...
---
Item Type: Enabler · Parent: E23 · Module: pilotage · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
