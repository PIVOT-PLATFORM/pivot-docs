# EN27.2 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « OKR » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `pilotage`** (`GET /api/pilotage/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine OKR)** :

- **Taux d'atteinte moyen des OKR** — score moyen des objectifs sur le cycle en cours (ratio 0.0–1.0 ; granularité : tenant/équipe) ; kpiKey `okr.taux-atteinte-moyen`
- **Part d'OKR à risque** — proportion d'objectifs en statut AT_RISK ou OFF_TRACK (pourcentage ; granularité : tenant/équipe) ; kpiKey `okr.part-a-risque`
- **Taux de check-ins réalisés** — part des KR ayant reçu un check-in sur la période attendue (pourcentage ; granularité : tenant/équipe) ; kpiKey `okr.taux-check-in`
- **Couverture d'alignement** — part des OKR rattachés à un objectif parent dans l'arbre d'alignement (pourcentage ; granularité : tenant/équipe) ; kpiKey `okr.couverture-alignement`

**Justification** : Exposer ces KPI méta permet de piloter la démarche OKR elle-même (adoption, cadence, alignement) depuis des KR ou des dashboards transverses, et de suivre la santé du dispositif sans requête ad hoc.
...
---
Item Type: Enabler · Parent: E27 · Module: pilotage · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
