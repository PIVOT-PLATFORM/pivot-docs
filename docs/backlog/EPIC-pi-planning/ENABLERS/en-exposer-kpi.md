# EN50.1 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « PI Planning » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `agilite`** (`GET /api/agilite/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine PI Planning)** :

- **Objectifs PI planifiés** — nombre d'objectifs de Program Increment définis pour le PI courant (nombre ; granularité : tenant/train) ; kpiKey `agilite.pi-objectifs-planifies`
- **Niveau de confiance PI** — confiance moyenne exprimée sur les objectifs du PI (échelle 1–5 ; granularité : tenant/train) ; kpiKey `agilite.pi-confiance`
- **Dépendances ouvertes** — nombre de dépendances inter-équipes non résolues sur le Program Board (nombre ; granularité : tenant/train) ; kpiKey `agilite.pi-dependances-ouvertes`
- **Risques identifiés** — nombre de tickets de type Risque placés sur le board (nombre ; granularité : tenant/train) ; kpiKey `agilite.pi-risques`

**Justification** : Exposer la confiance, les objectifs et les dépendances du PI permet de lier des KR de delivery à l'avancement réel du Program Increment et d'alimenter les cockpits de pilotage agile.
...
---
Item Type: Enabler · Parent: E50 · Module: agilite · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
