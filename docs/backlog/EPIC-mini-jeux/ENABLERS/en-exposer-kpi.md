...
- **Parties jouées** — nombre de parties de mini-jeux lancées sur la période (nombre ; granularité : tenant/équipe) ; kpiKey `minijeux.games_played`
- **Taux de participation** — part des membres invités ayant rejoint une partie (% ; granularité : équipe) ; kpiKey `minijeux.participation_rate`
- **Joueurs actifs** — nombre de participants distincts sur la période (nombre ; granularité : tenant/équipe) ; kpiKey `minijeux.active_players`
- **Taux de complétion des parties** — part des parties menées jusqu'au score final (% ; granularité : équipe) ; kpiKey `minijeux.completion_rate`
- **Engagement moyen** — nombre moyen d'interactions (coches, clics, réponses) par joueur et par partie (interactions/joueur ; granularité : équipe) ; kpiKey `minijeux.avg_engagement`
