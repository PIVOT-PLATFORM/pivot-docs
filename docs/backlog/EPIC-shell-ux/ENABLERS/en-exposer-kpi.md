# EN16.1 — Exposer les KPI du domaine (producteur KpiRef)

**Type d'enabler** : intégration

**Objectif technique** : Rendre les KPI du domaine « Shell applicatif & UX » **adressables et liables** par le système d'OKR (lien KR ↔ KPI interne, US27.8.3) et par tout futur consommateur, en exposant les KPI de cet EPIC via l'**endpoint KPI du module `core`** (`GET /api/core/kpi`) et l'événement `kpi.updated`, conformément au **contrat producteur de KPI EN28.14**. **Pas de FK inter-modules** (ADR-006) — références logiques (tenant + source + kpiKey + scope) uniquement.

**KPI exposés (domaine Shell applicatif & UX)** :

- **Taux d'adoption des modules** — part d'utilisateurs actifs ouvrant un module depuis la grille d'accueil (% ; granularité : tenant/équipe) ; kpiKey `shell.adoption_modules`
- **Temps de chargement du shell** — temps moyen de premier rendu du shell applicatif (ms ; granularité : tenant) ; kpiKey `shell.temps_chargement`
- **Score d'accessibilité** — score d'audit A11y des écrans du shell (score 0–100 ; granularité : tenant) ; kpiKey `shell.score_accessibilite`
- **Utilisateurs actifs** — utilisateurs distincts se connectant au portail sur la période (nombre ; granularité : tenant/équipe) ; kpiKey `shell.utilisateurs_actifs`
- **Adoption des thèmes** — part d'utilisateurs ayant personnalisé leur thème (% ; granularité : tenant) ; kpiKey `shell.adoption_themes`

**Justification** : Exposer l'adoption et l'expérience du shell comme KPI liables permet de rattacher des KR d'engagement, de performance perçue et d'accessibilité, et d'alimenter les tableaux de bord produit sans couplage inter-modules.
...
---
Item Type: Enabler · Parent: E16 · Module: core · Phase: phase-3 · Size: S · Priority: Medium
Stage: ⬜
Profils: Tous
